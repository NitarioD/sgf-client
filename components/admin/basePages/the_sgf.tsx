...
const EditAboutSGF = ({ pageToEdit }) => {
  ...
  const handleEdit = async () => {
    setLoading(true);
    const { data } = await axios.put(`/base_page/the_sgf`, {
  ...
  useEffect(() => {
    ...
    const getPointManPage = async () => {
      ...
      const removeHeadingTag = data.content
        .split("</h1>")
        .filter(
          (item: string) => item != "<h1>About The Scripture Grace Foundation"
        )
        .join("");
